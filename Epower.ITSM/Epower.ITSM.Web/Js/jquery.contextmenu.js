...
;(function($){
	...
	$.fn.contextMenu = function(menu,options) {	
	    ...
	    {
	        if (temptest.shown) {	        
	            if (temptest.iframe) { $(temptest.iframe).hide(); }
			    if (temptest.menu) { temptest.menu[temptest.hideTransition](temptest.hideSpeed,((temptest.hideCallback)?function(){temptest.hideCallback.call(temptest);}:null)); }
			    if (temptest.shadow) { temptest.shadowObj[temptest.hideTransition](temptest.hideSpeed); }
	        }
				
			temptest.shown = false; 
	    } 
