...
{
    public partial class CST_Issue_NoVisit : BasePage
    {
        #region 变量定义
        ...
        protected XmlDocument SqlWhereShow(string SQLName)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < SQLTextDetailList.Length; i++)
                {
                    ...
                    {
                        xmlEle = xmlDoc.CreateElement("Field");
                        xmlEle.SetAttribute("FieldName", "CustName");
                    ...
                    {
                        xmlEle = xmlDoc.CreateElement("Field");
                        xmlEle.SetAttribute("FieldName", "IssueNo");
                        xmlEle.SetAttribute("Value", SQLTextDetail[1].Trim());
                        xmlRoot.AppendChild(xmlEle);
                    }
                    #endregion
                    ...
                    {
                        xmlEle = xmlDoc.CreateElement("Field");
                        xmlEle.SetAttribute("FieldName", "ctrFCDWTType");
                        xmlEle.SetAttribute("Value", SQLTextDetail[1].Trim());
                        xmlRoot.AppendChild(xmlEle);
                    }
                    #endregion
                    ...
                    {
                        xmlEle = xmlDoc.CreateElement("Field");
                        xmlEle.SetAttribute("FieldName", "UserPSjzxr");
                        xmlEle.SetAttribute("Value", SQLTextDetail[1].Trim());
                        xmlRoot.AppendChild(xmlEle);
                    }
                    #endregion
                    ...
                    {
                        xmlEle = xmlDoc.CreateElement("Field");
                        xmlEle.SetAttribute("FieldName", "Person");
                        xmlEle.SetAttribute("Value", SQLTextDetail[1].Trim());
                        xmlRoot.AppendChild(xmlEle);
                    }
                    #endregion
                    ...
                    {
                        xmlEle = xmlDoc.CreateElement("Field");
                        xmlEle.SetAttribute("FieldName", "ServiceLevel");
                        xmlEle.SetAttribute("Value", SQLTextDetail[1].Trim());
                        xmlRoot.AppendChild(xmlEle);
                    }
                    #endregion
                    ...
                    {
                        xmlEle = xmlDoc.CreateElement("Field");
                        xmlEle.SetAttribute("FieldName", "Equ");
                        xmlEle.SetAttribute("Value", SQLTextDetail[1].Trim());
                        xmlRoot.AppendChild(xmlEle);
                    }
                    #endregion
                    ...
                    {
                        xmlEle = xmlDoc.CreateElement("Field");
                        xmlEle.SetAttribute("FieldName", "EmailState");
                        xmlEle.SetAttribute("Value", SQLTextDetail[1].Trim());
                        xmlRoot.AppendChild(xmlEle);
                    }
                    #endregion
        ...
        void Master_Master_Button_ExportExcel_Click()
        {
            ...
            string[] fileName = { "事件单号", "事件类别", "所属部门", "用户名称", "联系人", "办公电话", "联系地址", "资产名称", "发生时间", "报告时间", "完成时间", "服务级别", "影响度", "紧急度", "关闭理由", "事件来源", "事件状态", "摘要", "需求描述", "流程状态", "派出时间", "上门时间", "工程师", "措施及结果", "登单人" };
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            Session["FromUrl"] = "../AppForms/CST_Issue_NoVisit.aspx";
            ...
            if (this.IsShortCut == true)
            {
                ...
                string sreturnurl = "../AppForms/CST_Issue_NoVisit.aspx";
                ...
                if (strPara != string.Empty)
                {
                    if (Request["tblEmail"] != null && Request["tblEmail"].ToString() == "true")
                    {
                        sreturnurl = "../AppForms/CST_Issue_NoVisit.aspx?BookMark=7&IsShortCut=0&tblEmail=true";
                    }
                    else
                    {
                        sreturnurl = "../AppForms/CST_Issue_NoVisit.aspx?BookMark=7&IsShortCut=0";
                    }
                }
        ...
        protected void lkbOverTimeF_ExportExcel(string[] fileName, string[] arrFlide)
        {
           ...
           , long.Parse(Session["UserOrgID"].ToString()), tp, true, reTrace);
            Epower.ITSM.Web.Common.ExcelExport.ExportIssuesList1(this, dt, fileName, arrFlide, Session["UserID"].ToString());
        }
        ...
        private void IssueHighExportExcel(string[] fileName, string[] arrFlide)
        {
            ...
            DataTable dt = ZHServiceDP.GetIssuesForCond(xmlDoc.InnerXml, long.Parse(Session["UserID"].ToString()), long.Parse(Session["UserDeptID"].ToString()), long.Parse(Session["UserOrgID"].ToString()), reTrace);
            Epower.ITSM.Web.Common.ExcelExport.ExportIssuesList1(this, dt, fileName, arrFlide, Session["UserID"].ToString());
        }
        ...
        private void BindData()
        {
            ...
            switch (TypeID)
            {
                ...
                case "7":
                    ...
                    {
                        Session["FromUrl"] = "../AppForms/CST_Issue_NoVisit.aspx?BookMark=7";
                ...
                default://高级查询
                    ...
                    Session["Issue_List_Excel"] = "";
                    XmlDocument xmlDoc = SqlWhereShow(hidSQLName.Value);
                    dt = ee.GetIssuesForCond(xmlDoc.InnerXml, long.Parse(Session["UserID"].ToString()), long.Parse(Session["UserDeptID"].ToString()), long.Parse(Session["UserOrgID"].ToString()), reTrace, this.cpCST_Issue.PageSize, this.cpCST_Issue.CurrentPage, ref iRowCount);
                    break;
            }
            //设置显示字段
            if (!string.IsNullOrEmpty(this.HiddenColumn.Value))
        ...
        public string SetColumnDisplay(string arr)
        {
            ...
            {
                for (int i = 2; i <= this.gridUndoMsg.Columns.Count; i++)
                {
                    if (this.gridUndoMsg.Columns.Count < 14)
                        break;
                    else
                        this.gridUndoMsg.Columns.RemoveAt(2);
                }
                DataBing(arr);
            }
            return "";
        ...
        public void DataBing(string columns)
        {
            ...
            if (columnValues.Length > 1)
            {
                string[] key = columnValues[0].Split(',');
                string[] value = columnValues[1].Split(',');
                for (int i = key.Length - 1; i > -1; i--)
                {
                    if (key[i] != "")
                    {
                        BoundColumn bf = new BoundColumn();
                        bf.HeaderText = value[i];
                        bf.DataField = key[i];
                        bf.SortExpression = key[i];
                        bf.ItemStyle.Width = 150;
                        bf.HeaderStyle.Width = 150;

                        gridUndoMsg.Columns.AddAt(2, bf);
                    }
                }
            }
        }
