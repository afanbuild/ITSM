<%@ Page language="c#" Inherits="Epower.ITSM.Web.AppForms.frmpopSubject" Codebehind="frmpopSubject.aspx.cs" %>
					...
					if(value.length>1)
					{
						arr=value.split("@");
						 window.opener.document.getElementById("<%=Opener_ClientId %>txtPCatalogName").value=arr[1];
						  window.opener.document.getElementById("<%=Opener_ClientId %>hidPCatalogName").value=arr[1];
						   window.opener.document.getElementById("<%=Opener_ClientId %>hidPCatalogID").value=arr[0];
						   //更改父类后,进入配置编辑状态时 初始状态
						    window.opener.document.getElementById("<%=Opener_ClientId %>hidModified").value="true";
					}
				}
