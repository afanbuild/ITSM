...
{
    public partial class CST_Issue_AdvancedConditionNew : BasePage
    {
        ...
        /// 显示自定义字段
        /// </summary>
        public void ShowCustomizeColumn()
        {
            /* 服务请求 */
            Dictionary<String, String> dictServiceColumn = Dic(@"申请人,申请人地址,办公电话,电子邮件,
                                                                 详细信息,服务级别,派出时间,上门时间,
                                                                 执行人名称,完成时间,措施及结果"
                /*合计金额,合计工时*/,
                                                               @"#CustName,#CustAddress,#CTel,#Email,#Content,
                                                                 #ServiceLevel,#Outtime,#ServiceTime,#Sjwxr,
                                                                 #FinishedTime,#DealContent"
                /*TotalAmount,TotalHours*/);
            /* 事件请求 */
            Dictionary<String, String> dictEventColumn = Dic(@"客户名称,客户地址,办公电话,电子邮件,
                                                               详细信息,服务级别,事件来源,事件状态,
                                                               派出时间,上门时间,执行人名称,完成时间,
                                                               措施及结果"
                /*//,总计金额,合计工时,事件根源*/,
                                                             @"CustName,CustAddress,CTel,Email,Content,
                                                               ServiceLevel,ReSouseName,DealStatus,Outtime,
                                                               ServiceTime,Sjwxr,FinishedTime,DealContent"
                /*//IssueRootName,TotalAmount,TotalHours*/);

            String strVal = this.DropDownList1.SelectedValue;

            switch (strVal)
            {
                case "1":
                    this._TableColumnCheckBoxList.DataSource = dictEventColumn;
                    break;
                case "2":
                    this._TableColumnCheckBoxList.DataSource = dictServiceColumn;
                    break;
                default:
                    return;
            }
        ...
        protected void SqlWhereSaveTemp1(string SQLName)
        {
            ...
            if (IsExistQuery(" and Name='Temp1' and FORMID='CST_Issue_List' and  SQLWhere='Temp1' and LOGINNAME=" + StringTool.SqlQ(Session["UserName"].ToString())))
            {
                if (!string.IsNullOrEmpty(this.hidValue.Value))
                {
                    SQLstr = "update CST_ISSUE_QUERYSave set SQLText="
                            + StringTool.SqlQ(SQLText.ToString().Trim())
                            + ",DISPLAYCOLUMN=" + StringTool.SqlQ(this.hidValue.Value.ToString().Trim())
                            +
        ...
        protected void SqlWhereShow(string SQLName)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (!String.IsNullOrEmpty(strDisplayColumn)
                    && !strDisplayColumn.Equals("@"))
                {
                    //Content,Email,@详细信息,电子邮件,
                    String[] _strArray = strDisplayColumn.Split(new String[] { "@" },
                        StringSplitOptions.RemoveEmptyEntries);
                    // 详细信息,电子邮件,
                    String[] _strTextArray = _strArray[1].Split(new String[] { "," },
                        StringSplitOptions.RemoveEmptyEntries);
                    // Content,Email,
                    String[] _strValArray = _strArray[0].Split(new String[] { "," },
                        StringSplitOptions.RemoveEmptyEntries);
                    ...
                    // 设置显示类别和字段
                    DropDownList1.SelectedIndex = 0;/* 事件字段*/
                    if (_strValArray[0].Contains("#"))
                    { DropDownList1.SelectedIndex = 1;/*服务字段*/ }

                    ShowCustomizeColumn();

                    for (int i = 0; i < _strValArray.Length; i++)
                    {
                        lsbDeptTo.Items.Add(new ListItem(_strTextArray[i].Trim(),
                            _strValArray[i].Trim()));
                    }
                }
            }
