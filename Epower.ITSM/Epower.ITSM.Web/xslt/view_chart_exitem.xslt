...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"  xmlns:ms="urn:schemas-microsoft-com:xslt" xmlns:v="urn:schemas-microsoft-com:vml">
        ...
        <xsl:if test="LINKTEXT">
          <v:Rect filled="f" stroked="f">
            <xsl:attribute name="style">
              ...
              <xsl:value-of select="LINKTEXT/@X"></xsl:value-of>              
              <xsl:text>; top:</xsl:text>
              <xsl:value-of select="LINKTEXT/@Y"></xsl:value-of>              
            </xsl:attribute>
    ...
    <xsl:choose>
      ...
      <xsl:when test="USERDATA=70">
        <v:Rect filled="f" stroked="f">
          ...
          <table style="font-size:9pt; width:100%; height:100%; ">
            <tr>
              <td style="width:100%; height:100%; color:black; text-align:left; vertical-align:center; ">
      ...
      <xsl:otherwise>         
        <v:Rect filled="f"   stroked="f"> 
          ...
          <table style="font-size:9pt; width:100%; height:100%; ">
            <tr>
              <td style="width:100%; height:100%; cursor:pointer;color:blue; text-align:center; vertical-align:bottom; ">
                <xsl:attribute name="onclick">open_view_displayway(<xsl:value-of select="@NODEID"></xsl:value-of>);</xsl:attribute >
                <xsl:attribute name="id">
                  <xsl:text>T</xsl:text>
                  <xsl:value-of select="@NODEID"></xsl:value-of>
                </xsl:attribute>
                <xsl:value-of select="TEXT"></xsl:value-of><xsl:value-of select="LINKTEXT"></xsl:value-of>
              </td>
            </tr>
          </table>
        </v:Rect>
  ...
  <xsl:template name="Node_Image"> 
    <v:image>
      ...
      <xsl:attribute name="nodemodelid">
        <xsl:value-of select="@NODEID"></xsl:value-of>
      </xsl:attribute>
      ...
      <xsl:attribute name="style">
        <xsl:text>position:absolute; z-index:20; width:480; height:480; cursor:pointer;left:</xsl:text>
