...
namespace Epower.ITSM.Web.Forms
{
    ...
    /// frmRight 的摘要说明。
    /// </summary>
    public partial class frmRight : BasePage
    {
        ...
        private void LoadData(bool isall)
        {
            ...
            if (txtOpID.Text.Trim() != "" || txtOpName.Text.Trim() != "")
            {
                long lngOpID = 0;
                if (txtOpID.Text.Trim() != "")
                    lngOpID = StringTool.String2Long(txtOpID.Text);

                if (lngOpID != 0)
                    sWhere += "And a.OperateId=" + lngOpID.ToString();
                if (!string.IsNullOrEmpty(txtOpName.Text.Trim()))
                    sWhere += " and b.OpName like " + StringTool.SqlQ("%" + txtOpName.Text.Trim() + "%");
                dt = RightDP.GetRightList(sWhere, isall, strRangeID);
            }
            else if (txtObjectID.Text.Trim() != "" || dpdObjectType.SelectedValue != "0")
            {
