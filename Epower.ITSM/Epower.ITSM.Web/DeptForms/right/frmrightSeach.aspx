...
<%@ Page language="c#" Inherits="Epower.ITSM.Web.Forms.right.frmrightSeach" Codebehind="frmrightSeach.aspx.cs" %>
	...
	<HEAD id="HEAD1" runat="server">
		...
		<!--#include file="../../Js/tableSort.js" -->
			...
			function Show_ModifyWindow(obj)
			{
				...
				var rid=document.all(idtag+"hidRightId").value;
				var value=OpenNoBarWindow("../frmeditright.aspx?rightid="+rid+"&TypeFrm=frmrightSearch",500,300);
		...
		{
			var value=window.showModalDialog("frmPopOperates.aspx?OperateID="+document.all.txtOpID.value);
			if(typeof(value) != "undefined" && value.length>1)
			{
				arr=value.split("@");
				document.all.txtOpName.value=arr[1];
				document.all.txtOpID.value=arr[0];
				//alert(document.all.txtOperateId.value);
			}
			else
			{
			    document.all.txtOpName.value="";
				document.all.txtOpID.value="";
			}
		...
		{
		  //=====zxl===
		  var url="../frmpopdept.aspx?Opener_ClientId=<%=hidClientId_ForOpenerPage.ClientID %>&TypeFrm=frmrightSeach";
		...
		{
		//========zxl===
		    var url="../frmSelectStaffRight.aspx?Opener_ClientId=<%=hidClientId_ForOpenerPage.ClientID %>&TypeFrm=frmrightSeach";
		    window.open(url,"","scrollbars=no,status=yes ,resizable=yes,width=450,height=500,left=250,top=100");
		}
		...
		{
		    //=======zxl=====
		    var url="../frmPopActor.aspx?Opener_ClientId=<%=hidClientId_ForOpenerPage.ClientID %>&TypeFrm=frmrightSeach";
		    window.open(url,"","scrollbars=no,status=yes ,resizable=yes,width=500,height=520,left=250,top=100");
		}
	...
	<body bgcolor="#ffffff">
		<FORM id="Form1" method="post" runat="server">
			<TABLE id="tbMain" width="100%" class="listContent" align="center">
				...
				<TR height="30">
					<TD class="listTitle">
						<INPUT onclick="OpenNoBarWindow('../frmeditrightBatch.aspx?OpearId=<%=operateId%>&rightid=',600,500);" type="button"
				...
				<TR>
					<TD style="HEIGHT: 17px" class="listTitle">授权对象类型:
						<asp:dropdownlist id="dpdObjectType" runat="server"></asp:dropdownlist>
							...
							<Columns>
								...
								<asp:TemplateColumn>
									...
									<ItemTemplate>
										...
										<input type="hidden" id="hidRightId" runat="server" value='<%#DataBinder.Eval(Container.DataItem, "RightID")%>' />
									</ItemTemplate>
								</asp:TemplateColumn>
								<asp:TemplateColumn HeaderText="权限ID" Visible="false">
								...
								<asp:BoundColumn DataField="OpNameAndID" HeaderText="操作项" Visible=false>
