<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="browsepicAdvanced.ascx.cs"
    Inherits="Epower.ITSM.Web.Controls.browsepicAdvanced" %>
...
<div id="picRoot">
    <div id="pic1">
        ...
        <span id="spanPicName1">
            ...
            <asp:FileUpload ID="fileUPicture" runat="server" />
            <asp:HiddenField ID="hfPicOriginalURL" runat="server" />
            <br />
            <img id="preview_size_fake1" style="filter: progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod='image');
...
$(document).ready(function(){
    $('#<%=fileUPicture.ClientID %>').change(function(){
        var sender = this;
    ...
    var pictureParam = $('#<%=hfPicOriginalURL.ClientID %>');
    if (pictureParam.val() == '') { return;}    
    
    var paramArray = pictureParam.val().split('|');    
    var picUrl = window.location.href+'&tmpdir='+paramArray[0] + '&tmpfileid=' + paramArray[1];
