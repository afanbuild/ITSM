...
private string NoPhotosMessage = "该文件夹不存在或者是空的";
...
private string	DefaultImageFolder = "Attfiles\\Photos";  // 默认的起始文件夹
...
private void Page_Load(object sender, System.EventArgs e) {
	...
	if (isframe != "") {
		...
		if (cif != "" && rif != "") {
			RootPhotosFolder.Value = rif;
			CurrentPhotosFolder.Value = cif;
		} else {
			RootPhotosFolder.Value = DefaultImageFolder;
			CurrentPhotosFolder.Value = DefaultImageFolder;	
		}
			...
			if (UploadFile.PostedFile.FileName.Trim() != "")
            {
				if (IsValidFileType(UploadFile.PostedFile.FileName)) 
                {
					...
					try {
						...
						Session["PhotoName"]=UploadFileName;
						UploadFileDestination = HttpContext.Current.Request.PhysicalApplicationPath;
						UploadFileDestination += CurrentPhotosFolder.Value;
						...
						ResultsMessage.Text = "Your file could not be uploaded: " + ex.Message +UploadFileDestination +UploadFileName;
						//ResultsMessage.Text = UploadFailureMessage+Session["PhotoName"]+UploadFileDestination;
					}
				} 
                else
                {
...
public void DeleteImage_OnClick(object sender, EventArgs e) {
	if (FileToDelete.Value != "" && FileToDelete.Value != "undefined") {
		try {
			...
			System.IO.File.Delete(AppPath  + CurrentPhotosFolder.Value + "\\" + FileToDelete.Value);
...
private string[] ReturnFilesArray() {
	if (CurrentPhotosFolder.Value != "") {
		try {
			string AppPath = HttpContext.Current.Request.PhysicalApplicationPath;
			string ImageFolderPath = AppPath + CurrentPhotosFolder.Value;
...
private string[] ReturnDirectoriesArray() {
	if (CurrentPhotosFolder.Value != "") {
		try {
			string AppPath = HttpContext.Current.Request.PhysicalApplicationPath;
			string CurrentFolderPath = AppPath + CurrentPhotosFolder.Value;
...
public void DisplayPhotos() {
	...
	if ( (FilesArray == null || FilesArray.Length == 0) && (DirectoriesArray == null || DirectoriesArray.Length == 0) ) {
		gallerymessage.Text = NoPhotosMessage + ": " + RootPhotosFolder.Value+DirectoriesArray;
	} else {
		...
		if (CurrentPhotosFolder.Value != RootPhotosFolder.Value) {
			...
			myImageHolder.Attributes["ondblclick"]="gotoFolder('" + RootPhotosFolder.Value + "','" + ParentFolder.Replace("\\","\\\\") + "');";  
			myImageHolder.Controls.Add(myHtmlImage);

			System.Web.UI.WebControls.Panel myMainHolder = new System.Web.UI.WebControls.Panel();
			myMainHolder.CssClass = "Photospacer";
		...
		foreach (string _Directory in DirectoriesArray) {
			...
			try {
				...
				System.Web.UI.HtmlControls.HtmlImage myHtmlImage = new System.Web.UI.HtmlControls.HtmlImage();
				myHtmlImage.Src = AppUrl + "Photos/ftb/folder.big.gif";
				myHtmlImage.Attributes["unselectable"]="on"; 
				myHtmlImage.Attributes["align"]="absmiddle"; 
				...
				myImageHolder.Attributes["unselectable"]="on"; 
				myImageHolder.Attributes["onclick"]="divClick(this);";  
				myImageHolder.Attributes["ondblclick"]="gotoFolder('" + RootPhotosFolder.Value + "','" + DirectoryName.Replace(AppPath,"").Replace("\\","\\\\") + "');";  
				myImageHolder.Controls.Add(myHtmlImage);
				...
				System.Web.UI.WebControls.Panel myTitleHolder = new System.Web.UI.WebControls.Panel();
				myTitleHolder.CssClass = "titleHolder";
				myTitleHolder.Controls.Add(new LiteralControl(DirectoryName.Replace(AppPath + CurrentPhotosFolder.Value + "\\","")));
				myMainHolder.Controls.Add(myTitleHolder);
		...
		foreach (string ImageFile in FilesArray) {
			...
			try {
				...
				//galleryfilelocation += "/";
				ImageFileLocation += CurrentPhotosFolder.Value;
				ImageFileLocation += "/";
...
<tr><td height=16 style="padding-left:10px;border-top: 1 solid #999999; background-color:#99ccff;">
	...
	<tr>
		...
		<td valign=top><asp:button id="UploadImage" Text=" 上  传 " runat="server" onclick="UploadImage_OnClick"  /></td>
	...
	<input type="hidden" id="RootPhotosFolder" Value="Attfiles\\Photos" runat="server" />
	<input type="hidden" id="CurrentPhotosFolder" Value="Attfiles\\Photos" runat="server" NAME="CurrentPhotosFolder"/>
</td></tr>
</asp:panel>
</form>
</table>
